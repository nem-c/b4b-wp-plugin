...
<p><?php esc_html_e( 'We need the information below to complete your 100 hole registration. Please log into your account and update it as soon as you can.', 'b4b-theme-support' ); ?></p>
...
<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; margin-bottom: 40px;" border="1">
	<thead>
		<tr>
			...
			<th class="td" scope="col"><?php esc_html_e( 'Hat size', 'b4b-theme-support' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td class="td"><?php echo esc_html( $email->args['name'] ); ?></td>
			<td class="td"><?php echo esc_html( $email->args['email'] ); ?></td>

			<?php if ( empty( $email->args['phone'] ) ) : ?>
				<td class="td" style="background:red; color: #ffffff;"><?php esc_html_e( 'missing', 'b4b-theme-support' ); ?></td>
			<?php else: ?>
				<td class="td"><?php echo esc_html( $email->args['phone'] ); ?></td>
			<?php endif; ?>

			<?php if ( empty( $email->args['shirt_size'] ) ) : ?>
				<td class="td" style="background:red; color: #ffffff;"><?php esc_html_e( 'missing', 'b4b-theme-support' ); ?></td>
			<?php else: ?>
				<td class="td"><?php echo esc_html( $email->args['shirt_size'] ); ?></td>
			<?php endif; ?>

			<?php if ( empty( $email->args['hat_size'] ) ) : ?>
				<td class="td" style="background:red; color: #ffffff;"><?php esc_html_e( 'missing', 'b4b-theme-support' ); ?></td>
			<?php else: ?>
				<td class="td"><?php echo esc_html( $email->args['hat_size'] ); ?></td>
			<?php endif; ?>
		</tr>
	</tbody>
</table>
