...
class B4B_WC_Front_Donation {
	...
	public function add_golfers_dropdown( $data ) {
		...
		if ( empty( ( $golfers_ids_list ) ) === false ) {
			...
			$for = sanitize_text_field( $_REQUEST['for'] );
			if ( !ctype_digit( $for ) ) {
				$matching_golfers = get_users( [
					'meta_key'   => 'b4b_name_slug',
					'meta_value' => $for,
				] );
				if ( !empty( $matching_golfers ) && is_array( $matching_golfers ) ) {
					$for = $matching_golfers[0]->id;
				}
			}
			...
			echo '<label class="woocommerce-form__label woocommerce-form__label-for-checkbox" style="margin-bottom: 30px;">
					<input type="checkbox" name="b4b_dont_know_golfer" id="b4b_dont_know_golfer" class="woocommerce-form__input woocommerce-form__input-checkbox b4b_dont_know_golfer" style="width: auto;" value="1" /> <span>I don\'t know a golfer</span>
				</label>';
		}
	}
	...
	public function validate_donation_amount( $success, $product_id, $quantity ) {
		...
		if ( $product->get_type() === "donation" ) {
			if ( isset( $_REQUEST["golfer_id"] ) ) {
				...
				$dont_know_golfer = isset( $_POST['b4b_dont_know_golfer'] ) && $_POST['b4b_dont_know_golfer'] === '1';
				if ( $dont_know_golfer ) {
					$matching_golfers = get_users( [
						'search_fields' => 'display_name',
						'search'        => '_Birdies 4 Brains Donation',
					] );
					if ( count( $matching_golfers ) === 1 ) {
						$golfer_id = $matching_golfers[0]->ID;
					}
				}

				if ( $golfer_id < 1 ) {
					$success = false;
					wc_add_notice( __( "Please select a <strong>Golfer</strong>, if you don't know one pick any name and it will all go to the B4B cause :)", B4B_TEXT_DOMAIN ), "error" );
				}
			}
	...
	public function calculate_donation_cart_item_prices( $cart_item_data, $product_id, $variation_id, $quantity ) {
		...
		if ( $product->get_type() === "donation" ) {
			...
			if ( $dont_know_golfer ) {
				...
				if ( count( $matching_golfers ) === 1 ) {
					$donated_golfer_id = $matching_golfers[0]->ID;
				}
			}
			...
			$cart_item_data["dont_know_golfer"]    = $dont_know_golfer;
